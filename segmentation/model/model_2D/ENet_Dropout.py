...
import pickle
from model.model_2D.ENET.base_model import BaseModel
from model.model_2D.ENET.ops import dropout, max_unpool, PReLU


class ENet(BaseModel):
    def __init__(self, sess, conf):

        super(ENet, self).__init__(sess, conf)
        self.wd = 2e-4  # (weight decay)
        self.lr = 5e-4  # (learning rate)
        self.build_network(self.inputs_pl)
        self.class_weights = pickle.load(open("class_weights.pkl", 'rb'))
        self.configure_network()

    def build_network(self, x):
        # encoder:
        # # initial block:
        network = self.initial_block(x=x, scope="inital")
        ...
        network, pooling_indices_1 = self.encoder_bottleneck_regular(x=network,
                                                                     output_depth=64, keep_prob=1,
                                                                     scope="bottleneck_1_0", downsampling=True)
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_regular(x=network, output_depth=64,
                                                  keep_prob=1, scope="bottleneck_1_1")
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_regular(x=network, output_depth=64,
                                                  keep_prob=1, scope="bottleneck_1_2")
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_regular(x=network, output_depth=64,
                                                  keep_prob=1, scope="bottleneck_1_3")
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_regular(x=network, output_depth=64,
                                                  keep_prob=1, scope="bottleneck_1_4")
        print(network.get_shape().as_list())
        ...
        network, pooling_indices_2 = self.encoder_bottleneck_regular(x=network,
                                                                     output_depth=128, keep_prob=self.keep_prob_pl,
                                                                     scope="bottleneck_2_0", downsampling=True)
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_regular(x=network, output_depth=128,
                                                  keep_prob=self.keep_prob_pl, scope="bottleneck_2_1")
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_dilated(x=network, output_depth=128,
                                                  keep_prob=self.keep_prob_pl, scope="bottleneck_2_2",
                                                  dilation_rate=2)
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_asymmetric(x=network, output_depth=128,
                                                     keep_prob=self.keep_prob_pl, scope="bottleneck_2_3")
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_dilated(x=network, output_depth=128,
                                                  keep_prob=self.keep_prob_pl, scope="bottleneck_2_4",
                                                  dilation_rate=4)
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_regular(x=network, output_depth=128,
                                                  keep_prob=self.keep_prob_pl, scope="bottleneck_2_5")
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_dilated(x=network, output_depth=128,
                                                  keep_prob=self.keep_prob_pl, scope="bottleneck_2_6",
                                                  dilation_rate=8)
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_asymmetric(x=network, output_depth=128,
                                                     keep_prob=self.keep_prob_pl, scope="bottleneck_2_7")
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_dilated(x=network, output_depth=128,
                                                  keep_prob=self.keep_prob_pl, scope="bottleneck_2_8",
                                                  dilation_rate=16)
        print(network.get_shape().as_list())

        # layer 3:
        network = self.encoder_bottleneck_regular(x=network, output_depth=128,
                                                  keep_prob=self.keep_prob_pl, scope="bottleneck_3_1")
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_dilated(x=network, output_depth=128,
                                                  keep_prob=self.keep_prob_pl, scope="bottleneck_3_2",
                                                  dilation_rate=2)
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_asymmetric(x=network, output_depth=128,
                                                     keep_prob=self.keep_prob_pl, scope="bottleneck_3_3")
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_dilated(x=network, output_depth=128,
                                                  keep_prob=self.keep_prob_pl, scope="bottleneck_3_4",
                                                  dilation_rate=4)
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_regular(x=network, output_depth=128,
                                                  keep_prob=self.keep_prob_pl, scope="bottleneck_3_5")
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_dilated(x=network, output_depth=128,
                                                  keep_prob=self.keep_prob_pl, scope="bottleneck_3_6",
                                                  dilation_rate=8)
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_asymmetric(x=network, output_depth=128,
                                                     keep_prob=self.keep_prob_pl, scope="bottleneck_3_7")
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_dilated(x=network, output_depth=128,
                                                  keep_prob=self.keep_prob_pl, scope="bottleneck_3_8",
                                                  dilation_rate=16)
        print(network.get_shape().as_list())
        ...
        # fullconv:
        self.logits = tf.contrib.slim.conv2d_transpose(network, self.conf.num_cls,
    ...
    def encoder_bottleneck_regular(self, x, output_depth, scope, keep_prob,
        ...
        # # regularizer:
        conv_branch = dropout(conv_branch, keep_prob)
    ...
    def encoder_bottleneck_dilated(self, x, output_depth, keep_prob, scope,
    ...
    def encoder_bottleneck_asymmetric(self, x, output_depth, keep_prob, scope, proj_ratio=4):
        input_shape = x.get_shape().as_list()
        input_depth = input_shape[3]
