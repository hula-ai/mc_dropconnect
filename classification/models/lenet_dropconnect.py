...
class LeNet(BaseModel):
    ...
    def build_network(self, x):
        ...
        with tf.variable_scope('LeNet'):
            x = conv_2d(x, filter_size=5, num_filters=6, name='conv_1', keep_prob=self.keep_prob_pl)
            x = max_pool(x, 2, 2, 'pool_1')
            x = conv_2d(x, filter_size=5, num_filters=16, name='conv_2', keep_prob=self.keep_prob_pl)
            x = max_pool(x, 2, 2, 'pool_2')
            x = flatten_layer(x)
            x = fc_layer(x, 120, name='fc_1', keep_prob=self.keep_prob_pl)
            x = fc_layer(x, 84, name='fc_2', keep_prob=self.keep_prob_pl)
